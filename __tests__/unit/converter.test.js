...
describe('converter', () => {
  ...
  describe('stringToJson', () => {
    it('should convert string format to JSON', () => {
    ...
    it('should ignore invalid segments with no elements', () => {
...
<root>
  ...
  <AddressID>
    ...
    it('should handle multiple segments of same type', async () => {
...
<root>
  ...
  <ProductID>
    ...
    it('should handle empty XML root', async () => {
  ...
  describe('stringToXml', () => {
    it('should convert string directly to XML', () => {
...
<root>
  ...
  <AddressID>
    ...
    it('should convert XML directly to string', async () => {
...
<root>
  ...
  describe('round-trip conversions', () => {
    it('should maintain data integrity: String → JSON → String', () => {
    ...
    it('should maintain data integrity: JSON → XML → JSON', async () => {
      const xml = jsonToXml(expectedJsonData);
      const backToJson = await xmlToJson(xml);
      
      // XML conversion strips _metadata, so compare with clean version
      const expectedCleanJson = {
        "segments": [
          {
            "segment_id": "ProductID",
            "elements": ["4", "8", "15", "16", "23"]
      ...
      expect(backToJson).toEqual(expectedCleanJson);
    });

    it('should maintain data integrity: String → XML → String', async () => {
      const xml = stringToXml(testStringData, testSeparators);
      const backToString = await xmlToString(xml, testSeparators);
      
      const originalClean = testStringData.replace(/~$/, '');
      expect(backToString).toBe(originalClean);
    });
  });
});
